...
function toggleCheckbox(element) {
  ...
  else
  {
    ...
    if(checkedCnt === 0)state.checked=false;
  }
  postState();
}
function mode(){
  ...
  for(var i=0;i<collection.length;i++) mode += collection[i].checked ? "1" : "0"
...
function request4State(adress) 
{
  ...
  xhr.send();
  window.setTimeout(requestTimeout, 500);
}
...
function showMainSettings(state)
{
  ...
  {
    collection[i].checked = state.mode.slice(i + 1,i + 2) === "1";
  }
